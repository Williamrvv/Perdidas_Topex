...
if ($dia1!=""&&$dia2!=""&&$hora1!=""&&$hora2!="") {
	...
	if ($resultado = $con->query("SELECT COUNT(documentos.documento) FROM datosmaquina,documentos WHERE (datosmaquina.documento=documentos.documento) AND ((documentos.fecha >= '$dia1') AND (documentos.fecha<='$dia2')) AND ((documentos.hora >= '$hora1') AND (documentos.hora <= '$hora2') AND (documentos.cantidad>0))")) {
	  ...
	  <div class="col-md-6">
		  <div id="chart">
			  ...
			  <ul id="bars">
				...
				<?php 
					$consulta = "SELECT DISTINCT(`idmaquina`),maquina, COUNT(`idmaquina`) as cont  FROM datosmaquina, maquinas, documentos where (datosmaquina.idmaquina = maquinas.id)AND(datosmaquina.documento=documentos.documento)AND((documentos.fecha>='$dia1')AND(documentos.fecha<='$dia2'))AND((documentos.hora >= '$hora1')AND(documentos.hora<='$hora2') AND (documentos.cantidad>0))   GROUP BY idmaquina ORDER BY cont DESC";
		  ...
		  <div class="col-md-6">
			<ul>
				<li> Las 3 maquinas que han tenido más perdidas son: 
		  		<?php 
					$consulta = "SELECT 
						DISTINCT(`idmaquina`),maquina, COUNT(`idmaquina`) AS conteo FROM datosmaquina, maquinas, documentos 
					where 
						(datosmaquina.idmaquina =maquinas.id) AND (datosmaquina.documento=documentos.documento) AND ((documentos.fecha>='$dia1')AND(documentos.fecha<='$dia2')) AND ((documentos.hora >= '$hora1')AND(documentos.hora<='$hora2') AND (documentos.cantidad>0)) 
					GROUP BY 
						idmaquina 
					ORDER BY 
				...
				<li>El total de pérdidas hechas en Maquinas y en el rango seleccionado es de <strong style="color:red"><?php echo "$tdatper[0]"; ?></strong></li>
			</ul>
		  </div>
		  </div>
<br><br>
...
<table class="table table-bordered">
	...
	<?php 
		$consulta = "SELECT DISTINCT(`idmaquina`),maquina, COUNT(`idmaquina`) as cont, (COUNT(`idmaquina`)*100/$tdatper[0]) FROM datosmaquina, maquinas, documentos where (datosmaquina.idmaquina = maquinas.id) AND (datosmaquina.documento=documentos.documento) AND ((documentos.fecha>='$dia1')AND(documentos.fecha<='$dia2')) AND ((documentos.hora >= '$hora1')AND(documentos.hora<='$hora2') AND (documentos.cantidad>0)) GROUP BY idmaquina ORDER BY cont DESC";
		if ($resultado = $con->query($consulta)) {
			$index=1;
		    while ($fila = $resultado->fetch_row()) {
		    	@$acum+=$fila[3];
...
<div class="table-responsive" id="Exportar_a_Excel">
  ...
  <table class="table table-bordered dvDatos" >
	...
	<form action="php/ficheroexcel.php" method="post" target="_blank" id="FormularioExportacion">
		...
		<input type="hidden" id="datos_maquinas" name="datos_maquinas" />
	</form>
 
    <script>
		$(document).ready(function() {
			$("#botonExcel").click(function(event) {
				$("#datos_maquinas").val( $("<div>").append( $("#Exportar_a_Excel").eq(0).clone()).html());
